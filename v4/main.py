...
def send_restreamer_command(server_address, process_id, authorization_token, payload, is_snapshot=False):
    ...
    This function now constructs the API URLs dynamically and handles 401 errors.
    """
    ...
    try:
        ...
        if e.response is not None and e.response.status_code == 401:
            print("Received 401 Unauthorized. Attempting to refresh access token...")
            ...
            if new_access_token:
                config['authorization_token'] = new_access_token  # Update token in config
                headers['Authorization'] = f"Bearer {new_access_token}"  # Update headers
                # Retry the request with the new token
                try:
                    response = requests.put(command_url, headers=headers, json=payload)
                    response.raise_for_status()
                    print(
                        f"{payload['command'].capitalize()} command sent successfully after token refresh to process '{process_id}'. Response:"
                    )
                    print(response.json())
                except requests.exceptions.RequestException as e:
                    print(f"Error sending command to process '{process_id}' after token refresh: {e}")
            else:
                print("Error: Could not refresh access token. Command failed.")
        else:
